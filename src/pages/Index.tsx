...
const Index = () => {
  ...
  // CAMBIO: predictions son globales, no dependen de la liga seleccionada
  const { data: predictions = [] } = useMyPredictions();

  const predByMatch = useMemo(() => {
    const map = new Map<string, any>();
    for (const p of predictions) map.set((p as any).match_id, p);
    return map;
  }, [predictions]);
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-foreground">
      ...
      <main>
        ...
        {/* Selector de liga — solo afecta leaderboard y bonus bets, NO las predicciones */}
        {leagues.length > 1 && (
        ...
        <section id="predictions" className="container py-16">
          <div className="mb-8 flex items-end justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                ...
                <span className="text-primary font-medium">They count in all your leagues.</span>
              </p>
            </div>
          ...
          {upcoming.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No upcoming matches yet — run the sync-matches edge function to populate the schedule.
            </p>
          ) : (
                  ...
                  {recent.map((m: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {p && (
                          <span className="text-xs font-semibold shrink-0 ml-2 text-muted-foreground">
